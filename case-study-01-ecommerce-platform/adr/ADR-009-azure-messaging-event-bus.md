...
See [`../diagrams/azure-messaging-event-bus.png`](../diagrams/azure-messaging-event-bus.png) for the detailed diagram matching this ADR's Decision  the per-region Service Bus Premium namespace, one topic per event type with per-consumer subscriptions, session-ordering by order ID, and native dead-lettering, plus the full Proposed Configuration table. Checked against this ADR across several review rounds: an early draft listed the Fulfillment/Warehouse System under "Event Producers" while its own label said it consumes `order-confirmed` events  per `application-architecture.md` §4, Fulfillment/Warehouse only subscribes to events on this bus, it never publishes to it  moved to sit only under "Integrations," where it's shown correctly. The Trade-offs panel carried two bullets belonging to ADR-008 (DB peak sizing, a saga stuck mid-flight during a regional outage) rather than this ADR's own two trade-offs (Premium tier's fixed baseline cost, no single global view of order events across regions)  replaced. The References panel briefly carried ADR-010 and ADR-012, neither of which this ADR's text cites or has any relationship to  Order Orchestration's Service Bus consumer has no HTTP ingress and no dependency on the Identity Provider or API Gateway  and, after that fix, briefly self-referenced ADR-009; the panel was removed from the diagram entirely by request rather than corrected further.
