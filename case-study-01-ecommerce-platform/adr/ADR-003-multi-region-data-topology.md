...
**Trade-off:**
A customer whose home region fails over loses write availability for their in-flight cart or order until that region recovers  there is no cross-region failover for the regional-primary stores, unlike the catalog's replicated reads, which stay available everywhere even if the write region is unreachable. This is accepted explicitly: building cross-region write failover for cart/checkout/order data would mean either abandoning regional-primary partitioning (and the GDPR residency guarantee that comes with it) or building active-active multi-master writes with conflict resolution for financial transaction data  a materially riskier design this case study isn't taking on. Analytics or reporting that needs a consolidated, cross-region view of orders is not addressed by this topology and is out of scope for this design  a consequence of the regional-primary partitioning decided above, not a named exclusion from `problem-statement.md` §5.

**Status:** Approved

---

See [`../diagrams/multi-region-data-topology.png`](../diagrams/multi-region-data-topology.png) for the detailed diagram matching this ADR's Decision  the catalog's single-writer, multi-region read-replica topology (a distinct Catalog Primary in its home region, with every active region including that one served from its own read-only replica) and the three regional-primary transactional stores (US, EU, APAC), each with no cross-region replication to the others. Checked against this ADR across two review rounds: the first draft carried legend entries for card-data and payment-token flows that don't appear anywhere in this diagram (that mechanism is ADR-004's, not ADR-003's) and cited `problem-statement.md` §5 for the cross-region-analytics-is-out-of-scope note, which that section doesn't actually say  both corrected, with the citation pointed back to this ADR's own Trade-off paragraph above instead.
