...
See [`../diagrams/azure-network-topology.png`](../diagrams/azure-network-topology.png) for the detailed diagram matching this ADR's Decision — the three Secured Virtual Hubs (US, EU, APAC) connected automatically over Microsoft's backbone, each region's spoke VNet and its contents (ADR-005, ADR-006/ADR-007, ADR-008, ADR-012), and the full Proposed Configuration table. Checked against this ADR and its dependencies before being finalized: an early draft drew the Global Catalog replication as a chain — US Primary → EU Replica → APAC Replica — implying APAC's replica is fed from EU. Per ADR-007's Decision, both replicas read directly from the US primary independently; there is no EU→APAC hop. Redrawn as a fan-out, two separate replication arrows both originating from the US Primary box. The draft also placed Azure API Management in the "External/Global" panel as a single global resource reached via Private Link — but per ADR-012's Proposed Configuration, APIM is deployed one instance per active region, VNet-integrated into that region's spoke, not as a shared global instance; removed, since the correct per-region representation already exists inside each spoke ("Front Door / APIM Private Link"). Azure Front Door remained in the External/Global panel, since it genuinely is a single global resource.
