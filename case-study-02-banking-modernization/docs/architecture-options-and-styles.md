...
*Diagram source: [`diagrams/target-architecture-style.md`](../diagrams/target-architecture-style.md) (Mermaid, renders natively on GitHub — the authoritative, version-controlled diagrams-as-code source). A hand-drawn version matching the visual style of Case Studies 1 and 3 is now also available: [`diagrams/target-architecture-style.png`](../diagrams/target-architecture-style.png). It goes a layer further than the Mermaid source — folding in the Ledger-of-Intent state model and idempotency behavior from [Step 5](logical-design.md) ([ADR-003](../adr/ADR-003-provisional-vs-confirmed-state-model.md), [ADR-004](../adr/ADR-004-idempotency-and-exactly-once-delivery.md)) alongside this step's own ADR-001/ADR-002 decisions — and was checked component-by-component against both steps' documentation: all 9 logical components, all four ADR summaries, every NFR target, and every consequence/trade-off in ADR-001 and ADR-004 match. Both diagrams describe the same vendor-neutral design; platform choice is still deliberately undecided in both.*

## What Step 4 Deliberately Leaves Open

Which cloud platform hosts this new integration layer (Azure, AWS, GCP, or a private-cloud/VCF footprint) is **not** decided here — that is the entire point of running all four implementation tracks in Steps 6–9 before the Step 10 decision matrix. Step 4 fixes the *shape* of the solution (integration style, component disposition, build-vs-buy split); [Step 5](logical-design.md) restates that shape as a vendor-neutral logical design before any platform-specific service names enter the picture.
