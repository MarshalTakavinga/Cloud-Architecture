...
| 2 | [Banking modernization](case-study-02-banking-modernization/) | Core banking/payments workload  security, resiliency, regulatory reporting | **In progress** — Steps 1–9 of 13 complete: problem → current-state → requirements/NFRs → architecture options and target style (ADR-001 mainframe integration, ADR-002 build-vs-buy) → vendor-neutral logical design (ADR-003 reconciliation model, ADR-004 idempotency) → Azure implementation (ADR-005–ADR-010) → AWS implementation (ADR-011–ADR-016) → GCP implementation (ADR-017–ADR-022) → private-cloud implementation (ADR-023 platform/facility, ADR-024 Tanzu compute, ADR-025 self-managed PostgreSQL, ADR-026 NSX segmentation with zero hybrid-connectivity cost, ADR-027 direct AD join, ADR-028 self-managed RabbitMQ — all four implementation tracks now complete). Decision matrix (Step 10) is next. |
...
[Case Study 2 — Banking Modernization](case-study-02-banking-modernization/) is in progress (Steps 1–9 of 13 complete). [Case Study 3 — Healthcare Platform](case-study-03-healthcare-platform/) and [Case Study 1 — Global E-Commerce Platform](case-study-01-ecommerce-platform/) are both complete — see each case study's README for the full breakdown. Case Studies 4, 5, and 6 have not yet started.
