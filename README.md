...
| 2 | [Banking modernization](case-study-02-banking-modernization/) | Core banking/payments workload  security, resiliency, regulatory reporting | **In progress** — Steps 1–7 of 13 complete: problem → current-state → requirements/NFRs → architecture options and target style (ADR-001 mainframe integration, ADR-002 build-vs-buy) → vendor-neutral logical design (ADR-003 reconciliation model, ADR-004 idempotency) → Azure implementation (ADR-005–ADR-010) → AWS implementation (ADR-011 compute, ADR-012 data, ADR-013 messaging, ADR-014 hybrid connectivity, ADR-015 identity, ADR-016 multi-account landing zone — enrolling the existing 2021 account rather than rebuilding it). GCP implementation (Step 8) is next. |
...
[Case Study 2 — Banking Modernization](case-study-02-banking-modernization/) is in progress (Steps 1–7 of 13 complete). [Case Study 3 — Healthcare Platform](case-study-03-healthcare-platform/) and [Case Study 1 — Global E-Commerce Platform](case-study-01-ecommerce-platform/) are both complete — see each case study's README for the full breakdown. Case Studies 4, 5, and 6 have not yet started.
